...
var parse = ART.SVG.Parser.prototype.parse;
		...
		while (node){
			switch(node.nodeName){
				...
				case 'glyph':
					...
					if (d){
						// Flip path
						var path = new ART.Path(d), p = path.path;
						for (var i = 0, l = p.length; i < l; i++){
							var part = p[i];
							if (part[0] == 'V' || part[0] == 'v') part[1] = -part[1];
							for (var b = 2, m = part.length; b < m; b+=2){
								part[b] = -part[b];
							}
						}
						path.vml = null;
						glyph.d = path.toVML(1).substr(1);
					}
					glyphs[code] = glyph;
					break;
				// TODO: Kerning
